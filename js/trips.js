...
<<<<<<< HEAD
    ...
    const maxAlt = (trip.stats && trip.stats.maxAlt)
      ...
      : (trip.elevationProfile
          ? Math.max(...trip.elevationProfile.map(p => p.alt))
          : null);

    // Блок фітнес-статистики
    let statsHTML = '';
    if (trip.stats) {
      statsHTML = `
      ...
      statsHTML = `<div class="trip-no-stats">${isEn ? 'Detailed stats not available' : 'Детальна статистика не додана'}</div>`;
    }

    return `
      <div class="glass-card trip-card" onclick="openTripById('${trip.id}')" role="button" aria-label="${trip.title}" tabindex="0">
        ...
        <div class="trip-card-body">
          <div class="trip-key-metrics">
            <div class="trip-key-metric">
              <span class="trip-key-metric-value">${trip.durationDays} ${daysLabel}</span>
              <span class="trip-key-metric-label">${isEn ? 'duration' : 'тривалість'}</span>
            </div>
            <div class="trip-key-metric">
              <span class="trip-key-metric-value">${trip.distanceKm} ${kmLabel}</span>
              <span class="trip-key-metric-label">${isEn ? 'distance' : 'дистанція'}</span>
            </div>
            <div class="trip-key-metric">
              <span class="trip-key-metric-value">${maxAlt ? maxAlt + (isEn ? ' m' : ' м') : '—'}</span>
              <span class="trip-key-metric-label">${isEn ? 'max alt' : 'макс. вис.'}</span>
            </div>
          </div>
